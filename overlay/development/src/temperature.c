 ...
 *
 * @note
 * - 缓存文件路径为 /development/tmp/temperature.json，确保目录存在且可写。
 **********************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int init_serial(const char *port) {
    ...
    cfmakeraw(&options);
    cfsetspeed(&options, BAUD_RATE);// 设置波特率
...
// 解析响应字符串，提取湿度和温度
static int parse_response(const char *buf, float *humidity, float *temperature) {
    // 示例: "R:55.2RH 23.4C\r\n"
...
static int write_json_to_file(float humidity, float temperature) {
    int fd;
    char hum_str[32];
    char temp_str[32];
    json_object *root;
    ...
    written = write(fd, json_str, strlen(json_str));
    if (written < 0) {
        perror("write failed");
    }
    flock(fd, LOCK_UN);
